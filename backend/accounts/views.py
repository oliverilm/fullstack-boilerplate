...
from .utils.response import ResponseContent
...
def register_by_access_token(request, backend):
    ...
    return Response(
        ResponseContent.Error.InvalidToken,
...
def authentication_test(request):
    ...
    print(request.user)
    return Response(
        ResponseContent.Message.UserAuthenticated,
        status=status.HTTP_200_OK,
    )
...
class UpdateBasicUserPasswordView(UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        return Response(
            ResponseContent.Message.PasswordChanged, 
            status=status.HTTP_200_OK
        )
...
class SendResetOneTimeLinkView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                ResponseContent.Message.Success,
                status=status.HTTP_201_CREATED
            )
        except KeyError:
            return Response(ResponseContent.Error.EmailRequired, status=status.HTTP_400_BAD_REQUEST)
        except models.MyUser.DoesNotExist:
            return Response(
                ResponseContent.Error.InvalidEmail,
...
class ResetPasswordView(APIView):
    ...
    def post(self, request):
        ...
        if token is None or uidb64 is None:
            return Response(
                ResponseContent.Error.TokenAndUidb64Required,
                status=status.HTTP_400_BAD_REQUEST
            )
        try :
            ...
            user = None
            return Response(
                ResponseContent.Error.InvalidUser,
                status=status.HTTP_404_NOT_FOUND
            )
        if user is not None and password_reset_token.check_token(user, token):
            ...
            if serializer.is_valid():
                ...
                return Response(
                    ResponseContent.Message.PasswordResetted,
                    status=status.HTTP_200_OK
                )
            # case the serializer is not valid
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        # case the token is not recognized
        return Response(ResponseContent.Error.InvalidLink, status=status.HTTP_400_BAD_REQUEST)
